...
exports.TalkingBot = exports.getSuffix = exports.replaceAsync = exports.getTimeDifference = exports.Platform = void 0;
const twitch_1 = require("./twitch");
const discord_1 = require("./discord");
const youtube_1 = require("./youtube");
const kick_1 = require("./kick");
const beatsniper_js_1 = require("./beatsniper.js");
...
class TalkingBot {
    ...
    lastDynamicTitle;
    commandsFile = Bun.file(__dirname + "/../config/commands.json");
    aliasesFile = Bun.file(__dirname + "/../config/aliases.json");
    async readCustomCommands() {
        if (!(await this.commandsFile.exists()))
            return;
        this.customCommands = await this.commandsFile.json();
        if (!(await this.aliasesFile.exists()))
            return;
    ...
    constructor(kickId, server) {
        ...
        this.iochat.on("connect", () => {
            try {
                ...
                if (this.twitch.chatClient != null &&
        ...
        this.commandList = [
            {
                ...
                commandFunction: (data) => {
                    if (data.message.toLowerCase().includes("furry") &&
            ...
            },
            {
                ...
                commandFunction: (data) => {
                    if (data.message === "" || data.message === undefined) {
                        data.reply(getRandomElement(selfKillMessages).replaceAll("$1", data.user), false);
                    }
                    else {
                        data.reply(getRandomElement(killOtherMessages)
                            .replaceAll("$1", data.user)
                            .replaceAll("$2", data.message), false);
                    }
                },
            },
            {
                ...
                commandFunction: (data) => {
                    ...
                    if (data.platform == Platform.twitch) {
                        data.message = (0, twitch_1.parseTwitchEmotes)("!modtext " + data.message, data.context.emoteOffsets);
                        data.message = data.message.replace("!modtext", "");
                    }
                    else if (data.platform == Platform.kick) {
                        data.message = (0, kick_1.parseKickEmotes)(data.message);
                    }
                    this.modtext = data.message;
                    this.iomodtext.emit("message", this.modtext.replaceAll("$counter", this.counter.toString()));
                },
            },
            {
                showOnChat: false,
            ...
            },
            {
                ...
                commandFunction: async (data) => {
                    ...
                    if (stream == null) {
                        data.reply(`${this.twitch.channel.displayName} is currently offline`, true);
                        return;
                    }
                    const timeString = getTimeDifference(stream.startDate, new Date());
                    data.reply(`${this.twitch.channel.displayName} has been live for ${timeString}`, true);
                },
            },
            {
                showOnChat: false,
                ...
                commandFunction: async (data) => {
                    ...
                    data.reply(`\"${stream.title}\" - ${stream.gameName}: ${stream.tags}`, true);
                },
            },
            {
                showOnChat: false,
                ...
                commandFunction: async (data) => {
                    if (data.platform != Platform.twitch)
                        return;
                    const followed = await this.twitch.apiClient.channels.getChannelFollowers(this.twitch.channel.id, data.context.userInfo.userId);
                    // User is not following
                    if (followed.data.length == 0) {
                        data.reply(`You are not following ${this.twitch.channel.displayName}`, true);
                    }
                    else {
                        const timeString = getTimeDifference(followed.data[0].followDate, new Date());
                        data.reply(`@${data.user} has been following ${this.twitch.channel.displayName} for ${timeString}`, false);
                    }
                },
            },
            {
                ...
                commandFunction: (data) => {
                    ...
                    const response = data.message.substring(data.message.indexOf(" ") + 1, data.message.length);
            ...
            },
            {
                ...
                commandFunction: (data) => {
                    ...
                    data.reply(`Builtin Commands: ${builtin}, Custom Commands: ${custom}`, true);
                },
            },
            {
                showOnChat: false,
                command: "!settitle",
                commandFunction: async (data) => {
                    if (!data.isUserMod || data.message.length == 0)
                        return;
                    await this.twitch.apiClient.channels.updateChannelInfo(this.twitch.channel.id, { title: data.message });
            ...
            },
            {
                ...
                commandFunction: async (data) => {
                    if (!data.isUserMod || data.message.length == 0)
                        return;
                    const game = await this.twitch.apiClient.games.getGameByName(data.message);
                    if (game == null) {
            ...
            },
            {
                ...
                command: "!tags",
                commandFunction: async (data) => {
                    if (!data.isUserMod || data.platform !== Platform.twitch)
                        return;
                    const stream = await this.twitch.apiClient.streams.getStreamByUserId(this.twitch.channel.id);
                    if (stream == null) {
                    ...
                    switch (args[0]) {
                        case "add":
                            ...
                            await this.twitch.apiClient.channels.updateChannelInfo(this.twitch.channel.id, { tags: stream.tags.concat(newTags) });
                            data.reply(`Tags ${newTags} has been added`, true);
                            break;
                        case "remove":
                            const tagsToRemove = args.slice(1);
                            await this.twitch.apiClient.channels.updateChannelInfo(this.twitch.channel.id, {
            ...
            },
            {
                ...
                commandFunction: (data) => {
                    if (data.platform == Platform.twitch)
                        return;
                    this.twitch.chatClient.say(this.twitch.channel.name, `!bsr ${data.message}`);
                },
            },
            {
                showOnChat: true,
                command: "!tts",
                commandFunction: (data) => {
                    if (!data.isUserMod && !this.ttsEnabled)
                        return;
                    switch (data.platform) {
                        case Platform.twitch:
                            ...
                            this.iotts.emit("message", {
                                ...
                                parsedText: (0, twitch_1.parseTwitchEmotes)("!tts " + data.message, data.context.emoteOffsets).replace("!tts ", ""),
                            });
                            break;
                        case Platform.kick:
                            this.iotts.emit("message", {
                                text: (0, kick_1.removeKickEmotes)(data.message),
                                sender: data.user,
                                color: data.userColor,
                                parsedText: (0, kick_1.parseKickEmotes)(data.message),
                            });
                            break;
                        case Platform.youtube:
                            this.iotts.emit("message", {
                                ...
                                sender: data.user,
                                color: data.userColor,
                                parsedText: (0, youtube_1.parseYTMessage)(data.context),
                            });
                            break;
                    }
                },
            },
            {
                ...
                commandFunction: async (data) => {
                    ...
                    const songs = await (0, beatsniper_js_1.kill)(data.message);
                    if (songs.length === 0) {
                ...
                commandFunction: async (data) => {
                    switch (data.message) {
                        case "feed":
                            ...
                            break;
                        case "status":
                            this.pet.sayStatus(pet_1.StatusReason.command);
                            break;
                        case "graveyard":
                            this.pet.graveyard();
                            break;
                        ...
                        default:
                            if (data.platform == Platform.twitch)
                                data.reply("Usage !pet feed|fuel|status|graveyard. Use !petinfo for more info", true);
                    }
                },
            },
        ];
        this.pet = new pet_1.Pet(this);
