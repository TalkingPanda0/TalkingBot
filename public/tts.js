...
const voicesRemoveRegex = /^\((Jan|Giorgio|Geraint|Salli|Matthew|Kimberly|Kendra|Justin|Joey|Joanna|Ivy|Raveena|Aditi|Emma|Brian|Amy|Russell|Nicole|Kangkang|Linda|Heather|Sean)\)/
const voicesRegex = /(?<=^\()(Jan|Giorgio|Geraint|Salli|Matthew|Kimberly|Kendra|Justin|Joey|Joanna|Ivy|Raveena|Aditi|Emma|Brian|Amy|Russell|Nicole|Kangkang|Linda|Heather|Sean)(?=\))/
...
function handleQueue() {
    if (queue.length === 0 || isPlaying || messageList.childElementCount != 0) return;
    ...
    for (const key in emoteSoundEffects) {
        ...
        if (index !== -1) {
            ...
            if(index != 0){
                ...
                tempqueue.push(new Audio(`https://api.streamelements.com/kappa/v2/speech?voice=${message.voice}&text=${encodeURIComponent(text.slice(0,index))}`));
            }
            tempqueue.push(new Audio(emoteSoundEffects[key]));
            if(index+key.length != text.length){

                tempqueue.push(new Audio(`https://api.streamelements.com/kappa/v2/speech?voice=${message.voice}&text=${encodeURIComponent(text.slice(index+key.length,text.length))}`));
            }
    ...
    var audio = new Audio(`https://api.streamelements.com/kappa/v2/speech?voice=${message.voice}&text=${encodeURIComponent(text)}`);
