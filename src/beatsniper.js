async function getPlayerData(playerName) {
  return await (
    await fetch(`https://api.beatleader.xyz/players?search=${playerName}`)
  ).json();
}
...
async function getSongs(playerId) {
  return await (
    await fetch(
      `https://api.beatleader.xyz/player/${playerId}/scores?sortBy=date&order=desc`,
    )
  ).json();
}
...
async function getSongLeaderboardPosition(playerId, songId, diff, mode) {
  return await (
    await fetch(
      `https://api.beatleader.xyz/player/${playerId}/scorevalue/${songId}/${diff}/${mode}`,
    )
  ).json();
}
...
export async function kill(playerName) {
  try {
    ...
    while (songsToRequest.length < 3 && enemySongs.length != 0) {
      ...
      const myScore = await getSongLeaderboardPosition(
        playerId,
        songId,
        songDiff,
        songMode,
