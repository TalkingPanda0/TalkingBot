...
import { getTTSSounds } from "./tts.ts";
import fileUpload, { UploadedFile } from "express-fileupload";
import { createServer as createViteServer } from "vite";
import cors from "cors";
import { getDiscordUserId, isDiscordAuthData } from "../shared/util.ts";
import { handleKofiEvent, isKofiEvent } from "./kofi.ts";
import { MessageData } from "botModule";
import { readdir, rename } from "node:fs/promises";
import { CONFIG } from "./env.ts";
import { DiscordAuthData } from "../shared/types.ts";
...
app.use("/get", async (req, res) => {
  ...
  switch (req.path) {
    case "/messages": {
      ...
      const messages = await bot.chatLogger.getMessages(
...
app.get("/auth", async (req, res) => {
  ...
  res.set({
    "Set-Cookie": `discord_auth=${token};path=/api;max-age=${accessData.expires_in};HttpOnly;`,
    Location: "/control",
  });
  res.sendStatus(302);
});
...
async function listen() {
  if (process.env.NODE_ENV === "production") {
    ...
    app.get("/control/:page", (req, res, next) => {
      const file = path.resolve(
        __dirname,
        `../../dist/control/${req.params.page}.html`,
      );
