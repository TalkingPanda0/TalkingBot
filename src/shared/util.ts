...
import { DiscordAuthData } from "./types.ts";
...
export function isDiscordAuthData(
  obj: string | JwtPayload,
): obj is DiscordAuthData {
  return (
    typeof obj === "object" &&
    obj !== null &&
...
export function getDisplayName(display: string, login: string) {
...
export function formatDisplayName(message: ChatMessage) {
...
export function getUserColor(user: ChatUser): string {
  return user.color ? user.color : colorFromId(user.userId);
}
